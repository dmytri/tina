...
let stepPatterns = [];
let plankPatterns = [];
let pertubationMatches = [];

function extractPatterns(dir) {
  const patterns = [];
...
Given("the implementation directory contains no {string} token", t => {
  pertubationMatches = [];
  for (const d of ["packages/core/src", "packages/pi/src", "packages/opencode/src"]) {
    const p = resolve(ROOT, d);
    if (!existsSync(p)) continue;
    try {
      const r = execSync(`grep -rn "${t}" "${p}" --include='*.ts' 2>/dev/null || true`, { cwd: ROOT, encoding: "utf8" });
      if (r.trim()) pertubationMatches = pertubationMatches.concat(r.trim().split("\n").filter(Boolean));
    } catch {}
  }
});
...
Given("production seams carry @planks annotations", () => {
  plankPatterns = [];
  try {
    const r = execSync(`grep -rn '@planks\\|@planks-provisional' packages/ --include='*.ts' | grep -v dist/ || true`, { cwd: ROOT, encoding: "utf8" });
...
Then("the scan reports no matching tokens", () => {
  assert.equal(pertubationMatches.length, 0, `Found PERTURBATION tokens:\n${pertubationMatches.join("\n")}`);
});
...
Then("every @planks string matches a currently defined step-definition pattern", () => {
  ...
  assert.equal(unmatched.length, 0, `@planks patterns not matching any step definition:\n${unmatched.join("\n")}`);
});
...
Then("every behaviour-bearing step-definition pattern has a matching @planks annotation", () => {
  ...
  const uncovered = behaviourPatterns.filter(p => !plankPatterns.includes(p));
  assert.equal(uncovered.length, 0, `Step-definition patterns without a matching @planks:\n${uncovered.join("\n")}\nbehaviourFiles: ${JSON.stringify(behaviourFiles)}\nbehaviourPatterns: ${JSON.stringify(behaviourPatterns)}`);
});
